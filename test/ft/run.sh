...
#
# AT_FACTORIO    the game binary, if it is not where Steam puts it here
# AT_FT_DATA     the throwaway data directory the run happens in
#
...
factorio="${AT_FACTORIO:-/home/sparr/Games/Steam/steamapps/common/Factorio/bin/x64/factorio}"
...
data="${AT_FT_DATA:-$HOME/.cache/axial-tilt-factorio-test}"
...
# at-tests is the marker telling control.lua this is a test run. It is not on the mod
# portal, so the CLI cannot fetch it; put it where the CLI looks and it leaves it alone.
mkdir -p "$data/mods"
ln -sfn "$root/test/ft/at-tests" "$data/mods/at-tests"
